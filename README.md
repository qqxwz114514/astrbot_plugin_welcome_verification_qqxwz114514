...
> 原插件作者：月凌: https://github.com/qiyueling2716
...
- ✅ **用户退群/被踢自动停止验证**：验证过程中用户退群或被踢时，自动停止所有验证流程，避免不必要的验证操作
...
- **事件驱动检测**：监听 AstrBot 的 `group_decrease` 事件，当用户退群或被踢时立即停止验证流程
- **完整停止**：自动取消该用户的所有验证相关任务（验证任务、二级验证任务、超时踢人任务）
- **超时检查补充**：在验证超时后增加一次用户存在性检查，确保用户离开后及时停止
- **智能日志**：详细记录用户离开和停止验证流程的信息，便于调试和监控

#### 🔧 优化

- 验证失败处理前增加用户存在性检查，避免对已离开用户执行无意义操作
- 优化任务清理逻辑，确保资源正确释放
